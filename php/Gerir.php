<?php 
    ...
    $select = "SELECT * FROM `users` ORDER BY users.created_at DESC";
...
<html lang="en">
    ...
    <style>
        ...
        .right {
            ...
            padding: 15px;
            background-color: rgba(0, 0, 0, 0.9);
            border: 2px solid #429CAE;
            border-radius: 0px;
            box-shadow: 0px 0px 40px #429CAE inset;
        }
    ...
    <body>
        <div class="row">
            ...
            <div class="right">
                ...
                <h1 align="center" class="tit"> <b> Manage Accounts </b> </h1>
                ...
                <tr>
                    ...
                    <td style="text-align: center;"> <?php if($dados['is_admin'] == 1) echo '<font color="#CE8B24"> Yes </font>'; else echo '<font color="#808080"> No </font>'; ?> </td>
                ...
                <div style="text-align: center;">
                    <div class="card" style="margin-right: 2%; border-left: 4px solid #4263AE; border-radius: 5px;">
                        <p style="color: #4263AE;"> All Accounts </p>
                        <?php
                            $datas = "SELECT id FROM users ORDER BY id";
                            $result = mysqli_query($conn, $datas);
                            $row = mysqli_num_rows($result);
                            echo "<p style='text-align: center; font-size: 300%; color: #4263AE; margin-top: 2.5%; margin-bottom: 2.5%;'>" . $row . "</p>";
                        ?>
                    </div>
                    <div class="card" style="margin-right: 2%; border-left: 4px solid #CE8B24; border-radius: 5px;">
                        <p style="color: #CE8B24;"> Admin Accounts </p>
                        <?php
                            $datas = "SELECT id FROM users WHERE is_admin = 1 ORDER BY id";
                            $result = mysqli_query($conn, $datas);
                            $row = mysqli_num_rows($result);
                            echo "<p style='text-align: center; font-size: 300%; color: #CE8B24; margin-top: 2.5%; margin-bottom: 2.5%;'>" . $row . "</p>";
                        ?>
                    </div>
                    <div class="card" style="margin-right: 2%; border-left: 4px solid #808080; border-radius: 5px;">
                        <p style="color: #808080;"> Common Accounts </p>
                        <?php
                            $datas = "SELECT id FROM users WHERE is_admin = 0 ORDER BY id";
                            $result = mysqli_query($conn, $datas);
                            $row = mysqli_num_rows($result);
                            echo "<p style='text-align: center; font-size: 300%; color: #808080; margin-top: 2.5%; margin-bottom: 2.5%;'>" . $row . "</p>";
                        ?>
                    </div>
                    <div class="card" style="margin-right: 2%; border-left: 4px solid #42AE83; border-radius: 5px;">
                        <p style="color: #42AE83;"> Enable Accounts </p>
                        <?php
                            $datas = "SELECT id FROM users WHERE is_disable = 0 ORDER BY id";
                            $result = mysqli_query($conn, $datas);
                            $row = mysqli_num_rows($result);
                            echo "<p style='text-align: center; font-size: 300%; color: #42AE83; margin-top: 2.5%; margin-bottom: 2.5%;'>" . $row . "</p>";
                        ?>
                    </div>
                    <div class="card" style="border-left: 4px solid #AE4242; border-radius: 5px;">
                        <p style="color: #AE4242;"> Disable Accounts </p>
                        <?php
                            $datas = "SELECT id FROM users WHERE is_disable = 1 ORDER BY id";
                            $result = mysqli_query($conn, $datas);
                            $row = mysqli_num_rows($result);
                            echo "<p style='text-align: center; font-size: 300%; color: #AE4242; margin-top: 2.5%; margin-bottom: 2.5%;'>" . $row . "</p>";
                        ?>
                    </div>
                </div>

                <div id="overlay_disable">
                    <p style="margin-top: 10%; color: #939393; font-family: Century Gothic; font-size: 150%;"> ???????????????? </p>
                    <form action="Disable.php" method="POST">
                        <input type="hidden" name="disable_id" id="overlay_disable_input">
                        <button type="submit" name="disable_btn" class="disable_btn"> Yes </button>
                    </form>
                    <button onclick="off_disable()" class="disable_btn"> Cancel </button>
                </div>
